...
```bash
./scripts/install_app.sh 2.0.2 1
```

## Package Release Artifacts (`zip` + `dmg`)
```bash
./scripts/package_release.sh 2.0.2
```
...
Optional:
- skip in packaging: `SKIP_PII_SCAN=1 ./scripts/package_release.sh 2.0.2`
...
## Current Channel
`v2.0.2` is the active release channel with the premium redesign shell, adaptive sidebar, refreshed helper popup, and structured Performance workspace.
