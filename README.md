...
```bash
./scripts/install_app.sh 2.2.0 1
```
...
Optional:
- skip in packaging: `SKIP_PII_SCAN=1 ./scripts/package_release.sh 2.2.0 1`
...
## Current Channel
`v2.2.0 (build 1)` is the active release channel with the Calm Liquid Glass UI refactor, denser text-first Performance layouts, safer default cleanup exclusions for macOS state, Energy Mode controls, transparent Uninstaller fallback reporting, shared path-safety policy, bounded command execution, Full Disk Access diagnostics, and menu-bar helper reliability fixes.
